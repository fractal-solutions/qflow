...
class WebResearchAgentNode extends RobustAgentNode {
...
(async () => {
  ...
  const OPEN_ROUTER_MODEL = process.env.OPEN_ROUTER_MODEL || "openai/gpt-oss-20b:free";
  const OPEN_ROUTER_URL = process.env.OPER_ROUTER_URL || "https://openrouter.ai/api/v1";

  if (!OPEN_ROUTER_KEY) {
    console.warn("WARNING: OPEN_ROUTER_KEY is not set. Please set it to run this example.");
    return;
  }

  console.log('--- Starting Multi-Agent OpenRouter Example ---');

  // 1. Instantiate LLM for both agents (using OpenRouter)
  const openRouterLLM = new AgentOpenRouterLLMNode();
  openRouterLLM.setParams({
    apiKey: OPEN_ROUTER_KEY,
    model: OPEN_ROUTER_MODEL,
    baseUrl: OPEN_ROUTER_URL
  });

  // 2. Define tools available to the Web Research Agent
  const webResearchTools = {
    duckduckgo_search: new DuckDuckGoSearchNode(),
    web_scraper: new ScrapeURLNode(),
    
    system_notification: new SystemNotificationNode(),
    memory_node: new MemoryNode(),
  };

  // 3. Create the Web Research Agent instance
  const webResearchAgent = new WebResearchAgentNode(openRouterLLM, webResearchTools, openRouterLLM); // Use same LLM for summarization

  // 4. Wrap the Web Research Agent in an AsyncFlow to make it a sub-flow
  const webResearchAgentFlow = new AsyncFlow(webResearchAgent);
  // The prepAsync of the flow will pass the query from the conductor's shared state to the agent's goal
  webResearchAgentFlow.prepAsync = async (shared) => {
    if (shared.query) {
      webResearchAgent.setParams({ goal: shared.query });
    }
  };

  // 5. Define tools available to the Conductor Agent
  const conductorTools = {
    sub_flow: new AsyncNode(), // Placeholder, actual sub_flow logic is handled by AgentNode's execAsync
    
    system_notification: new SystemNotificationNode(),
    memory_node: new MemoryNode(),
  };

  // 6. Create a flow registry for the Conductor Agent
  const flowRegistry = {
    WebResearchAgentFlow: webResearchAgentFlow,
  };

  // 7. Create the Conductor Agent instance
  const conductorAgent = new ConductorAgentNode(openRouterLLM, conductorTools, openRouterLLM, flowRegistry);

  // 8. Set the initial goal for the Conductor Agent
  conductorAgent.setParams({
    goal: "Research the current state of quantum computing and provide a brief summary."
  });

  // 9. Create and run the main flow with the Conductor Agent
  const mainFlow = new AsyncFlow(conductorAgent);

  try {
    const finalResult = await mainFlow.runAsync({});
    console.log('\n--- Multi-Agent OpenRouter Example Finished ---');
    console.log('Final Result from Conductor Agent:', finalResult);
  } catch (error) {
    console.error('\n--- Multi-Agent OpenRouter Example Failed ---', error);
  }
})();
