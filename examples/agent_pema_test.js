...
import { PEMAAgent } from '../src/agent/pema_agent.js';
...
(async () => {
  ...
  console.log('--- Running Interactive PEMA Agent Test Workflow ---');
  ...
  const agent = new PEMAAgent(agentLLM, availableTools);
  ...
  try {
    ...
    console.log('\n--- Interactive PEMA Agent Test Workflow Finished ---');
    console.log('Final Agent Output:', finalResult);
  } catch (error) {
    console.error('\n--- Interactive PEMA Agent Test Workflow Failed ---', error);
  }
})();