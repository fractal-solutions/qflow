...
export class LPAVRLAgent extends BaseAgent {
    async execAsync() {
        ...
        if (!goal) {
            throw new Error("LPAVRLAgent requires a 'goal' parameter.");
        }
        ...
        while (step < this.maxSteps) {
            ...
            // Listen
            this.conversationHistory.push({ role: "user", content: "Current Phase: LISTEN. Understand the user’s current request and context." });
            const listenResponse = await this.getLLMAction();
            const { thought: listenThought } = this.parseLLMResponse(listenResponse);
            logger.thought(listenThought);
            this.conversationHistory.push({ role: "assistant", content: JSON.stringify({ thought: listenThought }) });

            // Plan
            this.conversationHistory.push({ role: "user", content: "Current Phase: PLAN. Break it into steps with rationale." });
...
` });
                ...
                // Visualize
                this.conversationHistory.push({ role: "user", content: "Current Phase: VISUALIZE. Present progress or results in an understandable form." });
            }
            const visualizeResponse = await this.getLLMAction();
            const { thought: visualizeThought } = this.parseLLMResponse(visualizeResponse);
            logger.thought(visualizeThought);
            this.conversationHistory.push({ role: "assistant", content: JSON.stringify({ thought: visualizeThought }) });

            // Reflect
            this.conversationHistory.push({ role: "user", content: "Current Phase: REFLECT. Judge results and align with the original intent." });
            ...
            // Learn
            this.conversationHistory.push({ role: "user", content: "Current Phase: LEARN. Update knowledge or strategy for future tasks." });
            const learnResponse = await this.getLLMAction();
            const { thought: learnThought } = this.parseLLMResponse(learnResponse);
