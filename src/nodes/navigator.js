...
export class NavigatorNode extends AsyncNode {
    ...
    async execAsync(prepRes, shared) {
        ...
        switch (action) {
            ...
            case 'navigate':
                ...
                return await this.requestJsonWithLegacyFallback(baseUrl, `/tabs/${encodeURIComponent(this.params.tabId)}/navigate`, '/navigate', 'POST', {
                    url: this.params.url,
            ...
            case 'snapshot': {
                ...
                return await this.requestJsonWithLegacyFallback(baseUrl, `/tabs/${encodeURIComponent(this.params.tabId)}/snapshot${query}`, `/snapshot${query}`);
            }
            case 'text': {
                this.requireParam('tabId');
                const query = buildQuery({ raw: this.params.raw });
                return await this.requestJsonWithLegacyFallback(baseUrl, `/tabs/${encodeURIComponent(this.params.tabId)}/text${query}`, `/text${query}`);
            }
            case 'evaluate':
                this.requireParam('tabId');
                this.requireParam('expression');
                return await this.requestJsonWithLegacyFallback(baseUrl, `/tabs/${encodeURIComponent(this.params.tabId)}/evaluate`, '/evaluate', 'POST', {
            ...
            case 'action':
                ...
                return await this.requestJsonWithLegacyFallback(baseUrl, `/tabs/${encodeURIComponent(this.params.tabId)}/action`, '/action', 'POST', this.params.interaction, {
                    tabId: this.params.tabId,
            ...
            case 'actions':
                ...
                return await this.requestJsonWithLegacyFallback(baseUrl, `/tabs/${encodeURIComponent(this.params.tabId)}/actions`, '/actions', 'POST', {
            ...
            case 'screenshot': {
                ...
                return await this.requestBinaryWithLegacyFallback(baseUrl, `/tabs/${encodeURIComponent(this.params.tabId)}/screenshot${query}`, `/screenshot${query}`, {
            ...
            case 'pdf': {
                ...
                return await this.requestBinaryWithLegacyFallback(baseUrl, `/tabs/${encodeURIComponent(this.params.tabId)}/pdf${query}`, `/pdf${query}`, {
    ...
    async requestJsonWithLegacyFallback(baseUrl, primaryRoute, legacyRoute, method = 'GET', body, legacyBody) {
        try {
            return await this.requestJson(baseUrl, primaryRoute, method, body);
        }
        catch (error) {
            const message = String(error?.message || '');
            if (legacyRoute && message.includes('HTTP 404') && message.includes(primaryRoute)) {
                return await this.requestJson(baseUrl, legacyRoute, method, legacyBody === undefined ? body : legacyBody);
            }
            throw error;
        }
    }
    ...
    async requestBinaryWithLegacyFallback(baseUrl, primaryRoute, legacyRoute, options = {}) {
        try {
            return await this.requestBinary(baseUrl, primaryRoute, options);
        }
        catch (error) {
            const message = String(error?.message || '');
            if (legacyRoute && message.includes('HTTP 404') && message.includes(primaryRoute)) {
                return await this.requestBinary(baseUrl, legacyRoute, options);
            }
            throw error;
        }
    }
